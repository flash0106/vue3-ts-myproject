...
import { TagView } from "@/typings/global";
...
function openContentMenu(tag: TagView, e: MouseEvent) {
  ...
  const contentMenuDom = document.getElementById("contextmenu");
  const offsetLeft = contentMenuDom?.getBoundingClientRect()?.left || 0;
  const offsetWidth = contentMenuDom?.offsetWidth || 0;
  const maxLeft = offsetWidth - menuMinWidth;
  const l = e.clientX - offsetLeft + 15; //15:margin-right
  if (l > maxLeft) {
    contentMenu.value.left = maxLeft;
  } else {
    contentMenu.value.left = l;
  }

  // 混合模式下，需要减去顶部菜单(fixed)的高度
  if (layout.value === "mix") {
    contentMenu.value.top = e.clientY - 50;
  } else {
    contentMenu.value.top = e.clientY;
  }
